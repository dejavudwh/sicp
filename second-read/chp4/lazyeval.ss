...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp)     ; 惰性求值修改的地方
         (apply-in-evaluators (actual-value (operator exp) env)
                              (operands exp)
                              env))
        (else
...
(define (apply-in-evaluators procedure arguments env)
  (cond ((primitive-procedure? procedure)
         (apply-primitive-procedure procedure
                                    (list-of-args-values arguments env))) ; lazy
        ((compound-procedure? procedure)
         (eval-sequence
           (procedure-body procedure)
           (extend-environment
             (procedure-parameters procedure)
             (list-of-delayed-args arguments env) ; lazy
