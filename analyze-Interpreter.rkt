(define apply-in-underlying-scheme apply) ;;基本过程的apply


(define (eval exp env) ;;eval
  ((analyze exp) env))


(define (analyze exp) ;;analyze
...
(define (analyze-self-evaluating exp) ;;self-evaluating
  (lambda (env) exp))

(define (analyze-quoted exp)  ;;quoted
  (let ((qval (text-of-quotation exp)))
    (lambda (env) qval)))

(define (analyze-variable exp) ;;variable
  (lambda (env) (lookup-variable-value exp env)))

(define (analyze-assignment exp) ;;assignment
...
(define (analyze-definition exp) ;;definition
...
(define (analyze-lambda exp)  ;;lambda
  (let ((vars (lambda-parameters exp))
        (bproc (analyze-sequence (lambda-body exp))))
    (lambda (env) (make-procedure vars bproc env))))


(define (analyze-sequence exps) ;;sequence
...
(define (analyze-application exp) ;;application
